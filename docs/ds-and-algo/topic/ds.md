...
顺序表（数组）是一种线性数据结构，元素在内存中连续存储。支持 $O(1)$ 随机访问，但插入和删除需要 $O(n)$ 移动后续元素。

大多数编程语言内置了顺序表，例如 C++ 的 `vector`、Python 的 `list`、Java 的 `ArrayList` 等。

## 链表

相较于 [顺序表](#顺序表) 在内存上是连续存储的，链表因其独特的结构在内存上是分散存储的，这就使得链表有着一些独特的性质：
...
而所谓的哈希表其实就是一个一维数组。哈希算法通过哈希函数，将「不可变的数据」映射为「一个数字（称为哈希码）」作为哈希表数组的下标，进而存储到哈希表中。由于数组是内存连续的，因此一旦知道哈希码，就可以 $O(1)$ 地在哈希表数组中索引出存储的内容。是典型的空间换时间策略。
