...
- 回环地址：网络号为 127，即 `127.0.0.0/8`。数据包不会离开本机，操作系统直接在内核层面转发给自己，主要用来本机网络服务测试，比如 `ping 127.0.0.1` 检测 TCP/IP 协议栈是否正常。

- 通配地址：全 0 地址，即 `0.0.0.0`。服务端表示绑定本机所有 IPv4 地址，外部可通过任何网卡 IP 访问；客户端还未获取 IP 时（如 DHCP 请求阶段），会用 `0.0.0.0` 表示本地地址；路由表中 `0.0.0.0` 表示默认路由，匹配所有未明确指定的目标网络。
